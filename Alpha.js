	...
	try {
		...
		function formatDate(n, locale = 'id') {
    	    ...
    	    if(time2 < "05:00:00"){
				...
				mess = {
					wait: 'Loading Proses',
					limit: 'Limit Mu Habis, Silahkan Beli Dengan Mengetik ${prefix}buylimit <nominal> Atau Minta Gift Dari User Lain',
...
Minat? Pm wa.me/${owner_number}`,
					ban: 'Maaf Kau Sudah Di Banned!',
					error: 'Warning! Error Proses Telah Terjadi',
					only: {
		...
		const promoteAdmin = async function(to, target=[]){
			...
			const getGroup = async function(totalchat){
				...
				download(url, namea, async function () {
					...
					const fakelokasi = {key: {fromMe: false, participant: `${numbernye}@s.whatsapp.net`, ...(from ? {remoteJid: "status@broadcast" } : {}) }, message: {locationMessage: {degreesLatitude: -7.0389318, degreesLongitude: 113.8969749, name: 'Rusia', address: '@HafR', jpegThumbnail: thumbnail}}}
					const fakekontak = {key: {fromMe: false, participant: `${numbernye}@s.whatsapp.net`, ...(from ? {remoteJid: "status@broadcast" } : {}) }, message: {contactMessage: {displayName: `${ucapannya}`, vcard: 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'N:Bot;Haf;BotId;;\n' + 'FN: Created By ©HafId\n' + 'item1.TEL;waid=6285706642632:+6285706642632\n' + 'item1.X-ABLabel:Telepon\n' + 'END:VCARD'}}}
...
_Tunggu Proses Upload Media_`
						...
						case 'wiki': case 'wikipedia':
									if (args.length < 1) return reply('Emang Mau Cari Apa?')
									reply(mess.wait)
									teks = args.join(' ')
...
Alasan : ${reason}`, [sender], true)
						...
						case 'premium': case 'prem':
									if (!isOwner && !Ofc.key.fromMe) return reply(mess.only.owner)
									if (args[0].startsWith('08')) return reply(`Example : ${prefix + command} add/dell 6285706642632`)
									if (args[0].startsWith("+62")) return reply(`Example : ${prefix + command} add/dell 6285706642632`)
...
💹 *EXPIRE :* ${cekvip.days} Hari, ${cekvip.hours} Jam, ${cekvip.minutes} Menit, ${cekvip.seconds} Detik`
						...
						case 'antibadword': case 'nobadword':
									...
									if (args[0] == 'on'){
										if (isBadword) return reply(`Udah aktifp`)
...
let qqppp = [{
                ...
                sendButLocation(from, teks, `HafBotId™© | Allright Reserved.`, thumbnail, qqppp, {contextInfo: { mentionedJid: [nupe]}})
...
break
						...
						case 'nulis': case 'tulis':
									...
									Zeeone.sendMessage(from, buff, image, {thumbnail: thumbnail, quoted: Ofc, caption: 'Jangan Males'})
									limitAdd(sender, limit)
									break
